...
GIT_TAG_COMMIT  ?= $$(git rev-parse --short --verify "$(VERSION)^{commit}" 2>/dev/null || git rev-parse --short HEAD)
...
IMG ?= kubelize/subst:$(VERSION)
PLUGIN_IMG ?= kubelize/subst-cmp:$(VERSION)
...
build: ## build golang binary
	@go build -ldflags "-X main.version=$(shell git describe --abbrev=0 --tags)" -o $(projectname) ./subst

.PHONY: install
install: ## install golang binary
	@go install -ldflags "-X main.version=$(shell git describe --abbrev=0 --tags)" ./subst

.PHONY: run
run: ## run the app
	@go run -ldflags "-X main.version=$(shell git describe --abbrev=0 --tags)" ./subst
...
clean: ## clean up environment
	...
	@test -f $(projectname) && rm $(projectname) || true
...
docker-build-cmp: ## build argocd plugin
	docker build . --platform linux/amd64 -f Dockerfile.argo-cmp -t ${PLUGIN_IMG} --build-arg GIT_HEAD_COMMIT=$(GIT_HEAD_COMMIT) \
 		--build-arg GIT_TAG_COMMIT=$(GIT_TAG_COMMIT) \
 		--build-arg GIT_MODIFIED=$(GIT_MODIFIED) \
 		--build-arg GIT_REPO=$(GIT_REPO) \
 		--build-arg GIT_LAST_TAG=$(VERSION) \
...
kind-load-image: PLUGIN_IMG = ghcr.io/kubelize/subst-cmp:local
